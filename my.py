...
#ANTIJS_V2
#cl = LineClient()
cl = LineClient(authToken='Ex5XkhNfswxQctiST3H9.3UrGMEjECFh9gbFk6EFekq.8Pg/3A3z7HGX/aPm2w5sJXHmUqahlOYlptg0gEFSzIo=')
cl.log("Auth Token : " + str(cl.authToken))
channel = LineChannel(cl)
cl.log("Channel Access Token : " + str(channel.channelAccessToken))

sw = LineClient(authToken='Exqud8xgFGO6Qncgdp6e.v0mtR6WhhyWt8ty6caAB3G.z+cuR2taSca4FPHRPg7Chpg+yXPUrr7zjK+1EdssDAg=')
...
wait = {
    ...
    "sticker":False,
    "selfbot":True,
    "mention":"SINI KAK GABUNG CHAT BARENG KITA KAK 😊",
    "Respontag":"Jembut di taq lagi kan bangke",
    "welcome":"Selamat datang & betah",
    "comment":"Like like & like by MORAYENA",
    "message":"Ketahuan lo add gw..mau apa lo BANGKE !!!",
    }
...
def mentionMembers(to, mid):
    try:
        ...
        cl.sendMessage(msg.to, None, contentMetadata={"STKID":"12772868","STKPKGID":"1316466","STKVER":"1"}, contentType=7)
        ...
        for i in mid:
            ...
            if no < len(mid):
                ...
                try:
                    ...
                    no = "\n╚══[ Success ]"
        cl.sendMessage(to, textx, {'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}, 0)
        cl.sendMessage(msg.to, None, contentMetadata={"STKID":"5418663","STKPKGID":"1132767","STKVER":"1"}, contentType=7)
    except Exception as error:
        cl.sendMessage(to, "[ INFO ] Error :\n" + str(error))
        ...
        for i in mid:
            ...
            if no < len(mid):
                ...
                try:
                    ...
                    no = "\n╚══[ Success ]"
        cl.sendMessage(to, textx, {'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}, 0)
        cl.sendMessage(msg.to, None, contentMetadata={"STKID":"5418662","STKPKGID":"1132767","STKVER":"1"}, contentType=7)
    except Exception as error:
        cl.sendMessage(to, "[ INFO ] Error :\n" + str(error))
    ...
    try:
        ...
        if op.type == 17:
            if op.param2 in wait["blacklist"]:
                cl.choice(ABC).kickoutFromGroup(op.param1,[op.param2])
            else:
                pass
               ...
               if msg._from not in Bots:
                 if wait["talkban"] == True:
                   if msg._from in wait["Talkblacklist"]:
                      try:
                          ramdom.choice(ABC).kickoutFromGroup(msg.to, [msg._from])
                      except:
                          try:
                              ...
                              Ticket = cl.reissueGroupTicket(op.param1)
                              sw.acceptGroupInvitationByTicket(op.param1,Ticket)
                              sw.choice(ABC).kickoutFromGroup(msg.to, [msg._from])
                              sw.leaveGroup(op.param1)
                              X = cl.getGroup(op.param1)
                              X.preventedJoinByTicket = True
                              cl.updateGroup(X)
                 ...
                 if wait["detectMention"] == True:
                   ...
                   for mention in mentionees:
                        if mention ['M'] in Bots:
                           ...
                           cl.sendMessage(msg.to, None, contentMetadata={'mid': "MORA,'"}, contentType=13)
                           cl.sendMessage(msg.to, None, contentMetadata={"STKID":"12772868","STKPKGID":"1316466","STKVER":"1"}, contentType=7)
                           break
                    ...
                    if text is None:
                        ...
                        if cmd == "self on":
                          ...
                          if wait["selfbot"] == True:
                            ...
                            if msg._from in admin:
                                cl.sendText(msg.to,"Creator Bot MORAYENA") 
                                ma = ""
                                for i in creator:
                                    ma = cl.getContact(i)
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                               ...
                               for target in targets:
                                   if target not in Bots:
                                       try:
                                           cl.choice(ABC).kickoutFromGroup(msg.to, [target])
                                       except:
                                           pass
